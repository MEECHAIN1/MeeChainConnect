...
const SAPPHIRE_CONFIG = {
  ...
  chainId: '0x5afe', // Sapphire Mainnet (23294)
  ...
  const handleConnection = async (web3auth: Web3Auth) => {
    try {
      ...
      setState(prev => ({
        ...
        isCorrectNetwork: Number(chainId) === SAPPHIRE_CONFIG.chainId.startsWith('0x') 
          ? parseInt(SAPPHIRE_CONFIG.chainId, 16) 
          : 23294,
      }));
    } catch (error) {
